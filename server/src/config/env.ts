...
const requiredBase64Key32 = (key: string) => {
...
const requireStrongInProduction = (key: string, minLength = 32) => {
...
const parseKekMap = (primaryId: string, primaryKeyB64: string) => {
...
export const env = {
  ...
  AI_RECONCILIATION_MISMATCH_THRESHOLD: Number(
    process.env.AI_RECONCILIATION_MISMATCH_THRESHOLD ?? "500"
  ),
  AI_RECONCILIATION_HOURLY_CRON_MS: Number(
    process.env.AI_RECONCILIATION_HOURLY_CRON_MS ?? "3600000"
  ),
  AI_RECONCILIATION_DAILY_CRON_MS: Number(
    process.env.AI_RECONCILIATION_DAILY_CRON_MS ?? "86400000"
  ),
  ...
  AI_CB_P95_LATENCY_THRESHOLD_MS: Number(
    process.env.AI_CB_P95_LATENCY_THRESHOLD_MS ?? "10000"
  ),
  ...
  OBS_SPEND_HOURLY_USD_THRESHOLD: Number(
    process.env.OBS_SPEND_HOURLY_USD_THRESHOLD ?? "50"
  ),
  ...
  AI_CREDIT_DRAIN_ALERT_THRESHOLD: Number(
    process.env.AI_CREDIT_DRAIN_ALERT_THRESHOLD ?? "250"
  ),
  AI_PROMPT_COST_USD_PER_1K: Number(process.env.AI_PROMPT_COST_USD_PER_1K ?? "0.001"),
  AI_COMPLETION_COST_USD_PER_1K: Number(
    process.env.AI_COMPLETION_COST_USD_PER_1K ?? "0.002"
  ),
  ...
  SUBSCRIPTION_RECONCILIATION_ENABLED:
    (process.env.SUBSCRIPTION_RECONCILIATION_ENABLED ?? "1").trim() === "1",
  SUBSCRIPTION_RECONCILIATION_INTERVAL_MS: Number(
    process.env.SUBSCRIPTION_RECONCILIATION_INTERVAL_MS ?? "86400000"
  ),
  SUBSCRIPTION_RECONCILIATION_BATCH_SIZE: Number(
    process.env.SUBSCRIPTION_RECONCILIATION_BATCH_SIZE ?? "100"
  ),
  ...
  SECURITY_AUDIT_VERIFY_INTERVAL_MS: Number(
    process.env.SECURITY_AUDIT_VERIFY_INTERVAL_MS ?? "21600000"
  ),
  MESSAGE_KEK_B64,
  MESSAGE_KEK_ID,
...
if (env.NODE_ENV === "production" && env.SECURITY_STRICT_MODE) {
  if (env.CORS_ORIGIN === "*") {
    throw new Error("CORS_ORIGIN must not be '*' in production");
  }

  if (!startsWithAny(env.CORS_ORIGIN, ["https://"])) {
  ...
  if (![301, 302, 307, 308].includes(env.HTTPS_REDIRECT_CODE)) {
  ...
  if (env.AUDIT_SIGNING_SECRET === "change-audit-secret") {
  ...
  if (env.CALLING_ENABLED) {
    if (!env.LIVEKIT_URL || !startsWithAny(env.LIVEKIT_URL, ["wss://"])) {
      throw new Error("LIVEKIT_URL must use wss:// in production when CALLING_ENABLED=1");
    }
    if (!env.LIVEKIT_API_KEY || !env.LIVEKIT_API_SECRET) {
      throw new Error(
        "LIVEKIT_API_KEY and LIVEKIT_API_SECRET are required in production when CALLING_ENABLED=1"
      );
    }
  }

  if (env.TURN_URL && !startsWithAny(env.TURN_URL, ["turns:"])) {
    throw new Error("TURN_URL must use turns: in production if configured");
  }

  const providerBaseUrls: Array<[string, string]> = [
    ["OPENAI_BASE_URL", env.OPENAI_BASE_URL],
    ["OPENROUTER_BASE_URL", env.OPENROUTER_BASE_URL],
    ["ANTHROPIC_BASE_URL", env.ANTHROPIC_BASE_URL],
    ["GEMINI_BASE_URL", env.GEMINI_BASE_URL],
    ["XAI_BASE_URL", env.XAI_BASE_URL],
  ];
  ...
  const mustNotBePlaceholder: Array<[string, string]> = [
    ["JWT_SECRET", env.JWT_SECRET],
    ["AUDIT_SIGNING_SECRET", env.AUDIT_SIGNING_SECRET],
    ["HEALTH_TOKEN", env.HEALTH_TOKEN],
    ["MESSAGE_KEK_B64", env.MESSAGE_KEK_B64],
  ];
  if (env.CALLING_ENABLED) {
    mustNotBePlaceholder.push(
