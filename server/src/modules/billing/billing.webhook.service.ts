...
const mapRazorpayStatus = (status: string): SubscriptionStatus => {
  const normalized = status.trim().toLowerCase();
  ...
  if (normalized === "halted" || normalized === "pending") return "PAST_DUE";
  return "PAST_DUE";
};
...
const defaultPlanId = async () => {
  ...
  if (!plan) throw new AppError(500, "No plan found for subscription bootstrap");
  return plan.id;
};
...
const isOutOfOrderEvent = async (input: {
  ...
  if (!input.providerSubscriptionId || !input.providerCreatedAt) return false;
  const latest = await (prisma as any).subscriptionWebhookEvent.findFirst({
    where: {
  ...
  if (!latest?.providerCreatedAt) return false;
  return input.providerCreatedAt.getTime() < (latest.providerCreatedAt as Date).getTime();
};
...
const createEventIfNew = async (input: {
  ...
  try {
    const created = await (prisma as any).subscriptionWebhookEvent.create({
      data: {
        provider: input.provider,
    ...
    return { event: created, duplicate: false as const };
  } catch (error) {
    if (error instanceof Error && /Unique constraint failed/i.test(error.message)) {
      await (prisma as any).subscriptionWebhookEvent.updateMany({
...
const updateEventStatus = async (
  ...
  await (prisma as any).subscriptionWebhookEvent.update({
    where: { id },
    data: {
      status: input.status,
...
const verifyStripeSignature = (rawBody: Buffer, signatureHeader: string) => {
...
const processStripeSubscriptionState = async (input: {
  ...
  if (eventRow.duplicate) return { accepted: true, duplicate: true };
  if (!eventRow.event) return { accepted: true, duplicate: true };
  ...
  const currentPeriodStart =
    currentPeriodStartRaw > 0 ? new Date(currentPeriodStartRaw * 1000) : new Date();
  const currentPeriodEnd =
    currentPeriodEndRaw > 0
...
const processStripeInvoiceState = async (input: { event: StripeEvent; object: Record<string, unknown> }) => {
  ...
  const mappedStatus =
    paid || statusRaw === "paid" ? ("ACTIVE" as SubscriptionStatus) : ("PAST_DUE" as SubscriptionStatus);
...
const handleStripeWebhook = async (input: {
  ...
  const parsed = input.parsedBody as StripeEvent;
  ...
  if (!parsed || typeof parsed !== "object" || !parsed.type) {
    throw new AppError(400, "Invalid Stripe webhook payload");
  }

  const object = (parsed.data?.object ?? {}) as Record<string, unknown>;
...
const verifyRazorpaySignature = (rawBody: Buffer, signatureHeader: string) => {
...
const handleRazorpayWebhook = async (input: {
  ...
  const subEntity = (providerSubscription.entity ?? {}) as Record<string, unknown>;
  ...
  if (created.duplicate) return { accepted: true, duplicate: true };
  if (!created.event) return { accepted: true, duplicate: true };
  if (!signatureVerified) {
    await updateEventStatus(created.event.id, {
      status: "ERROR",
  ...
  const metadata = (subEntity.notes ?? {}) as Record<string, unknown>;
  const metadataUserId = typeof metadata.userId === "string" ? metadata.userId : undefined;
  const metadataPlanId = typeof metadata.planId === "string" ? metadata.planId : undefined;
  const resolved = await resolveUserAndPlan({
    provider: "RAZORPAY",
...
const fetchStripeSubscription = async (providerSubscriptionId: string) => {
...
const fetchRazorpaySubscription = async (providerSubscriptionId: string) => {
...
const reconcileSubscriptionsWithProviders = async (input?: { limit?: number }) => {
  const limit = Math.max(1, Math.min(1000, input?.limit ?? env.SUBSCRIPTION_RECONCILIATION_BATCH_SIZE));
