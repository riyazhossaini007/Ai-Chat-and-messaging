...
import { alertDeliveryService } from "./alertDelivery.service";

type ProviderModelKey = `${string}::${string}`;
type OpsAlertTypeKey =
  | "SPEND_SPIKE"
  | "ERROR_SPIKE"
  | "PROVIDER_DOWNTIME"
  | "BILLING_ANOMALY"
  | "ESTIMATOR_SPIKE"
  | "DLQ_GROWTH";
type OpsAlertSeverityKey = "LOW" | "MEDIUM" | "HIGH" | "CRITICAL";
type OpsAlertStatusKey = "OPEN" | "ACKNOWLEDGED" | "RESOLVED";

const toKey = (provider: string, model: string): ProviderModelKey => `${provider}::${model}`;
...
const ensurePricingBootstrap = async () => {
  const count = await (prisma as any).providerPricing.count();
  if (count > 0) return;
  const now = new Date();
  const seeds = [
    {
      provider: "openai",
      model: "openai",
      inputUsdPer1M: env.AI_PROMPT_COST_USD_PER_1K * 1000,
      outputUsdPer1M: env.AI_COMPLETION_COST_USD_PER_1K * 1000,
      effectiveFrom: now,
      pricingVersion: 1,
    },
    {
  ...
  await (prisma as any).providerPricing.createMany({ data: seeds });
};

const upsertAlert = async (input: {
  type: OpsAlertTypeKey;
  severity: OpsAlertSeverityKey;
  dedupeKey: string;
  message: string;
  ...
  const existing = await (prisma as any).opsAlert.findUnique({
    where: { dedupeKey: input.dedupeKey },
  });
  if (existing && existing.status !== "RESOLVED") {
    return existing;
  }
  const alert = await (prisma as any).opsAlert.upsert({
    where: { dedupeKey: input.dedupeKey },
    create: {
      type: input.type,
  ...
  await alertDeliveryService
    .deliverAlert({
      ...
      provider: (alert.provider as string | null) ?? null,
      model: (alert.model as string | null) ?? null,
      details: (alert.details as unknown) ?? {},
      triggeredAt: alert.triggeredAt as Date,
    })
    .catch(() => undefined);
  return alert;
};

const getLiveMetrics = async (input?: { hours?: number }) => {
  ...
  const paywallBlocks = await (prisma as any).securityEvent.count({
    where: {
      type: "PAYWALL_BLOCK",
      createdAt: { gte: since },
    },
  });
  const providerHealth = await (prisma as any).aiProviderHealth.findMany();
  ...
  for (const row of logs) {
    ...
    if (row.status === "ERROR") bucket.error += 1;
    ...
    if (typeof row.ttftMs === "number") bucket.ttfts.push(row.ttftMs);
    if (typeof row.streamDurationMs === "number") bucket.streamDurations.push(row.streamDurationMs);
...
const runDailyRollup = async (input?: { date?: Date }) => {
  ...
  const ledger = await prisma.creditLedger.findMany({
    where: {
      ...
      type: { in: [CreditLedgerType.DEBIT, CreditLedgerType.AI_USAGE, CreditLedgerType.REFUND] },
    },
    select: {
      provider: true,
      model: true,
  ...
  const getBucket = (provider: string, model: string) => {
  ...
  for (const row of ledger) {
    ...
    if (row.type === CreditLedgerType.REFUND) {
  ...
  for (const bucket of buckets.values()) {
    const revenueUsd = bucket.creditsCharged / Math.max(1, env.AI_CREDITS_PER_USD);
    const marginUsd = revenueUsd - bucket.costUsd;
    const marginPct = revenueUsd > 0 ? marginUsd / revenueUsd : 0;
    await (prisma as any).dailyAiCost.upsert({
      where: {
        date_provider_model: {
          date: start,
          provider: bucket.provider,
...
const evaluateAlerts = async () => {
  ...
  const [hourLogs, dayLogs, healthRows, anomalousCharged, hourRefunds, hourLedger, dlqCount] = await Promise.all([
    ...
    (prisma as any).aiProviderHealth.findMany(),
    ...
    prisma.creditLedger.aggregate({
      where: {
        createdAt: { gte: oneHourAgo },
        type: { in: [CreditLedgerType.DEBIT, CreditLedgerType.AI_USAGE] },
      },
      _count: { _all: true },
    }),
    (prisma as any).backgroundJob.count({
      where: {
        status: "DLQ",
      },
    }),
  ...
  const baselineSpend =
    dayLogs.length > 0 ? dayLogs.reduce((sum, row) => sum + row.costUsd, 0) / 24 : 0;

  if (
    totalSpendHour > env.OBS_SPEND_HOURLY_USD_THRESHOLD ||
    (baselineSpend > 0 && totalSpendHour > baselineSpend * env.OBS_SPEND_SPIKE_MULTIPLIER)
  ) {
    await upsertAlert({
      type: "SPEND_SPIKE",
      severity: "HIGH",
      ...
      details: {
        totalSpendHour,
        baselineSpend,
        threshold: env.OBS_SPEND_HOURLY_USD_THRESHOLD,
      },
    });
  }

  const grouped = new Map<ProviderModelKey, { total: number; errors: number; latencies: number[] }>();
  ...
  for (const [key, bucket] of grouped) {
    ...
    if (errorRate > env.OBS_ERROR_RATE_THRESHOLD) {
      await upsertAlert({
        type: "ERROR_SPIKE",
        severity: "HIGH",
    ...
    if (p95 > env.OBS_PROVIDER_P95_MS_THRESHOLD) {
      await upsertAlert({
        type: "ERROR_SPIKE",
        severity: "MEDIUM",
        ...
        provider,
        model,
        details: { p95, threshold: env.OBS_PROVIDER_P95_MS_THRESHOLD, window: "1h" },
      });
    }
  }
  ...
  const estimatorRate =
    hourLogs.length > 0 ? hourLogs.filter((row) => row.estimatorUsed).length / hourLogs.length : 0;
  if (estimatorRate > env.OBS_ESTIMATOR_RATE_THRESHOLD) {
    await upsertAlert({
      type: "ESTIMATOR_SPIKE",
      severity: "MEDIUM",
      dedupeKey: `estimator-spike-${new Date().toISOString().slice(0, 13)}`,
      message: "Estimator usage spike detected",
      details: { estimatorRate, threshold: env.OBS_ESTIMATOR_RATE_THRESHOLD },
    });
  }

  const refundRate =
    (hourLedger._count._all ?? 0) > 0
      ? (hourRefunds._count._all ?? 0) / (hourLedger._count._all ?? 1)
      : 0;
  if (refundRate > env.OBS_REFUND_RATE_THRESHOLD) {
    await upsertAlert({
      type: "BILLING_ANOMALY",
      severity: "HIGH",
      dedupeKey: `refund-rate-spike-${new Date().toISOString().slice(0, 13)}`,
      message: "Refund rate spike detected",
      details: { refundRate, threshold: env.OBS_REFUND_RATE_THRESHOLD },
    });
  }
  if (dlqCount > env.OBS_DLQ_ALERT_THRESHOLD) {
    await upsertAlert({
      type: "DLQ_GROWTH",
      severity: "HIGH",
      dedupeKey: `dlq-growth-${new Date().toISOString().slice(0, 13)}`,
      message: "DLQ job count exceeded threshold",
      details: { dlqCount, threshold: env.OBS_DLQ_ALERT_THRESHOLD },
    });
  }
...
const listAlerts = async (input?: { limit?: number; status?: OpsAlertStatusKey }) => {
  return (prisma as any).opsAlert.findMany({
...
const acknowledgeAlert = async (id: string) => {
  return (prisma as any).opsAlert.update({
    where: { id },
...
const startOpsWorkers = () => {
  if (!alertTimer) {
    ...
    }, Math.max(10000, env.OBS_ALERT_EVAL_INTERVAL_MS));
  }
  if (!rollupTimer) {
    rollupTimer = setInterval(() => {
      void runDailyRollup().catch(() => undefined);
    }, Math.max(60_000, env.OBS_DAILY_ROLLUP_INTERVAL_MS));
  }
};
