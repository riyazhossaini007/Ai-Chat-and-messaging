...
import {
  ...
  emitAiTurnUpdatedToUser,
} from "../../socket";
import { creditsService } from "../credits/credits.service";
import { aiService, type AiChatInputMessage, type ChatModel } from "./ai.service";
import type { AiJobInput } from "./ai-thread.types";

const FRIENDLY_ATTACHMENT_ERROR =
  "I can't read this file yet. Please upload text or use supported formats.";

const sanitizeText = (value: string) =>
  value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");

const estimateTokensFromText = (value: string) => Math.max(1, Math.ceil(value.trim().length / 4));

const calculateCostUsd = (promptTokens: number, completionTokens: number) => {
...
const resolveModelForUser = async (requesterId: string): Promise<ChatModel> => {
  const hasPaid = await creditsService.hasActiveSubscription(requesterId);
  const preferred = hasPaid ? ("openai" as const) : ("openrouter" as const);
...
const buildSystemPrompt = (input: {
  commandType: AiJobInput["commandType"];
  translateTo?: string;
}) => {
  ...
  const contextBlock =
    input.contextMessages.length === 0
      ...
      : input.contextMessages
          .map(
            (item) =>
              `- (${item.id}) ${item.senderId ?? "unknown"}: ${(item.content ?? "").slice(0, 500)}`
          )
          .join("\n");
...
export const processAiJob = async (jobId: string) => {
  ...
  const input = job.input as AiJobInput;
  ...
  try {
    ...
    const targetContent =
      target && !target.deletedForEveryone
    ...
    const unsupportedAttachment =
      Boolean(target?.mediaUrl) && (!targetContent || !targetContent.trim());
    if (unsupportedAttachment) {
      ...
      emitAiTurnUpdatedToUser(thread.requesterId, {
        threadId: thread.id,
        turn: toTurnPayload(failedTurn),
      });
      emitAiThreadUpdatedToUser(thread.requesterId, {
        threadId: thread.id,
    ...
    const aiMessages: AiChatInputMessage[] = [
      {
        role: "system",
    ...
    const selectedVersion = aiService.listProviderVersions(selectedModel)[0];
    ...
    const completionTokens =
      completion.usage.completionTokens || estimateTokensFromText(completion.text);
