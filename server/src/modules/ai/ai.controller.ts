...
import {
  aiService,
  type AiChatInputMessage,
  type AiCompletionUsage,
  type ChatModel,
} from "./ai.service";
import { reliabilityService } from "./reliability.service";
import { promptGuardService } from "../security/promptGuard.service";
import { securityService } from "../security/security.service";
import { redisRateLimiterService } from "../security/redisRateLimiter.service";
...
const mapAiUsageProvider = (provider: string) =>
  provider.toLowerCase().includes("openrouter") ? "OPENROUTER" : "OPENAI";
...
const toRole = (value: unknown): AiChatInputMessage["role"] | null => {
...
const parseMessages = (value: unknown): AiChatInputMessage[] => {
  ...
  const parsed = value.map((item) => {
    if (!item || typeof item !== "object" || Array.isArray(item)) {
      throw new AppError(400, "Each message must be an object");
    }
    ...
    if (!role) {
      throw new AppError(400, "Message role must be one of: system, user, assistant");
    }
  ...
  if (parsed.length === 0) {
    throw new AppError(400, "messages cannot be empty");
  }

  return parsed;
};

const parseTemperature = (value: unknown) => {
...
const parseMaxTokens = (value: unknown) => {
  ...
  if (
    ...
    value < 1 ||
    value > env.AI_MAX_TOKENS_PER_REQUEST
  ) {
    throw new AppError(
      400,
      `maxTokens must be an integer between 1 and ${env.AI_MAX_TOKENS_PER_REQUEST}`
    );
  }
  return value;
};
...
const parseRequestId = (value: unknown) => {
  if (typeof value !== "string") {
    throw new AppError(400, "requestId is required and must be a UUID string");
  }
  ...
  if (!uuidPattern.test(trimmed)) {
    throw new AppError(400, "requestId must be a valid UUID");
  }
  return trimmed;
};
...
const parseRegeneratedFromRequestId = (value: unknown) => {
  if (typeof value !== "string") return undefined;
  const trimmed = value.trim();
  ...
  const uuidPattern =
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
  if (!uuidPattern.test(trimmed)) {
    throw new AppError(400, "regeneratedFromRequestId must be a valid UUID");
  }
  return trimmed;
};
...
const estimatePromptTokens = (messages: AiChatInputMessage[]) => {
  return messages.reduce((sum, item) => sum + estimateTokensFromText(item.content), 0);
};

const finalizeUsage = (
  providerUsage: AiCompletionUsage | null | undefined,
  input: {
...
const calculateCostUsd = (usage: AiCompletionUsage) => {
  const promptCost = (usage.promptTokens / 1000) * env.AI_PROMPT_COST_USD_PER_1K;
  const completionCost =
    (usage.completionTokens / 1000) * env.AI_COMPLETION_COST_USD_PER_1K;
...
const sendCreditsRequired = (res: Response) => {
...
const sendModelCooldown = (res: Response, waitMs: number) => {
...
const sendRateLimited = (res: Response, retryAfterMs: number) => {
  return res.status(429).json({
    success: false,
    error: "RATE_LIMITED",
...
const sendModelNotAllowed = (
  res: Response,
  input: { allowedSelections: string[]; defaultSelection: string }
) => {
...
const sendProviderDegraded = (
  res: Response,
  input: { provider: string; suggestedModels: string[]; switchedTo?: string | null }
) => {
...
const isProviderDegradedError = (error: unknown) => {
...
const isCreditsRequiredError = (error: unknown) => {
  if (!(error instanceof AppError)) return false;
  if (error.details && typeof error.details === "object") {
    const details = error.details as { error?: string; code?: string };
    return details.error === "CREDITS_REQUIRED" || details.code === "CREDITS_REQUIRED";
  }
  return false;
};

const isModelCooldownError = (error: unknown) => {
  if (!(error instanceof AppError)) return false;
  if (!error.details || typeof error.details !== "object") return false;
  const details = error.details as { error?: string };
...
const isModelNotAllowedError = (error: unknown) => {
  if (!(error instanceof AppError)) return false;
  if (!error.details || typeof error.details !== "object") return false;
  const details = error.details as { error?: string };
...
const getRateLimitRetryAfterMs = (error: unknown) => {
  if (!(error instanceof AppError)) return 0;
...
const getModelCooldownWaitMs = (error: unknown) => {
  if (!(error instanceof AppError)) return 0;
  if (!error.details || typeof error.details !== "object") return 0;
  const details = error.details as { retryAfterMs?: number };
...
const resolveAllowedModelSelections = async (userId: string) => {
  const enabledModels = aiService.listEnabledModels();
  const allSelections = enabledModels.flatMap((provider) =>
    aiService.listProviderVersions(provider).map((version) => `${provider}:${version}`)
  );
  const hasPaidOrComplimentaryAiAccess = await creditsService.hasActiveSubscription(userId);
  if (hasPaidOrComplimentaryAiAccess) {
    return {
      subscriptionActive: true as const,
      allowedSelections: allSelections,
      defaultSelection: aiService.getDefaultSelection(),
      allSelections,
    };
  }
  const freeSelections = allSelections.filter((selection) => aiService.isFreeSelection(selection));
  if (freeSelections.length === 0) {
    throw new AppError(503, "No free AI model versions are configured.");
  }

  return {
    subscriptionActive: false as const,
...
const resolveAllowedModelSelection = (
  selectedSelection: string,
  allowedSelections: string[],
  defaultSelection: string
) => {
  if (allowedSelections.includes(selectedSelection)) return selectedSelection;
  return defaultSelection;
};

const filterAllowedSuggestions = (suggestedModels: string[], allowedSelections: string[]) => {
  const allowedProviders = new Set(
    allowedSelections.map((selection) => selection.split(":")[0]).filter(isChatModel)
  );
  return suggestedModels.filter((item): item is ChatModel => {
    const model = item.trim().toLowerCase();
    return isChatModel(model) && allowedProviders.has(model);
  });
};

const enforceAiRateLimit = async (req: AuthRequest, messages: AiChatInputMessage[]) => {
  const userId = req.user?.id ?? "unknown";
  const ip = req.ip ?? "unknown";
  const estimated = estimatePromptTokens(messages);
  const result = await redisRateLimiterService.consumeAiBudgets({
    userId,
    ip,
  ...
  if (!result.allowed) {
    await securityService.logSecurityEvent({
      ...
      requestId: String((req.body as Record<string, unknown>)?.requestId ?? ""),
...
const enforceModelCooldown = (userId: string, model: ChatModel) => {
  if (model !== "claude" && model !== "grok") return;
  const now = Date.now();
  const key = `${userId}:${model}`;
  const nextAllowedAt = modelCooldowns.get(key) ?? 0;
  if (nextAllowedAt > now) {
    const waitMs = nextAllowedAt - now;
    throw new AppError(429, `Try again in ${Math.ceil(waitMs / 1000)}s`, {
      error: "MODEL_COOLDOWN",
      code: "MODEL_COOLDOWN",
      retryAfterMs: waitMs,
      model,
  ...
  modelCooldowns.set(key, now + env.AI_REASONING_COOLDOWN_MS);
};
...
const loadAiConfigRow = async () => {
  ...
  const admin = aiService.getAdminConfig();
  return (prisma as any).aiConfig.create({
    data: {
      id: "singleton",
      ...
      limits: {
        ...
        maxTokensPerRequest: env.AI_MAX_TOKENS_PER_REQUEST,
      },
      cooldowns: {
        reasoningCooldownMs: env.AI_REASONING_COOLDOWN_MS,
      },
      version: 1,
    },
  });
...
export const postAiChat = async (
  ...
  const regeneratedFromRequestId = parseRegeneratedFromRequestId(
    body.regeneratedFromRequestId
  );
  ...
  try {
    ...
    const existing = await prisma.aiRequestLog.findUnique({
      where: { requestId },
      select: {
        userId: true,
        status: true,
    ...
    if (existing) {
      if (existing.userId !== req.user.id) {
        throw new AppError(409, "requestId already exists for a different user");
      }
      if (existing.status === "RUNNING") {
        throw new AppError(409, "Request is already in progress");
      }
    ...
    const guarded = await traceService.withSpan(
      "ai.context_build",
      { requestId, traceId, chatId: chatId ?? null },
      async () => {
        const rawMessages = parseMessages(body.messages);
        return promptGuardService.enforcePromptGuardrails(rawMessages);
      }
    );
    ...
    let selected = aiService.parseModelSelection(rawModel);
    let selectedModel = selected.provider;
    let selectedVersion = selected.version;
    let selectedSelection = selected.selection;
    const allowedSelection = resolveAllowedModelSelection(
      selectedSelection,
      modelAccess.allowedSelections,
      modelAccess.defaultSelection
    );
    if (allowedSelection !== selectedSelection) {
      selected = aiService.parseModelSelection(allowedSelection);
    ...
    await traceService.withSpan("ai.auth_and_gate", { requestId, traceId }, async () => {
      if (modelAccess.subscriptionActive) {
        await creditsService.assertCanUseAi(userId);
      }
    });
    ...
    const preflight = await reliabilityService.checkProviderBeforeRequest(selectedModel);
    if (preflight.blocked) {
      const fallback = await reliabilityService.resolveFallbackModel(selectedModel);
      if (
        fallback.mode === "AUTO" &&
        fallback.fallbackModel &&
        modelAccess.allowedSelections.some((selection) =>
          selection.startsWith(`${fallback.fallbackModel}:`)
        )
      ) {
        console.info(
          JSON.stringify({
        ...
        selectedVersion = aiService.listProviderVersions(selectedModel)[0];
        ...
        console.warn(
          JSON.stringify({
            ...
            suggestedModels: filterAllowedSuggestions(
    ...
    if (existing && (existing.status === "ERROR" || existing.status === "CANCELLED")) {
      await prisma.aiRequestLog.update({
        ...
        data: {
          ...
          finishedAt: null,
          estimatorUsed: false,
          providerUsageRaw: Prisma.JsonNull,
          startedAt: new Date(),
        },
      });
    } else {
    ...
    const result = await traceService.withSpan(
      "ai.provider_call",
      {
        requestId,
        traceId,
    ...
    await reliabilityService.recordRequestSuccess(selectedModel, Date.now() - requestStartedAtMs);
    ...
    if (usage.totalTokens < estimatedTokenBudget) {
      await redisRateLimiterService.refundTokenBudget({
        userId: req.user.id,
    ...
    await traceService.withSpan("ai.response_write", { requestId, traceId }, async () => {
      await prisma.aiRequestLog.update({
        where: { requestId },
        data: {
          traceId,
          ...
          providerUsageRaw: result.providerUsageRaw
            ? (result.providerUsageRaw as Prisma.InputJsonValue)
            : undefined,
          costUsd,
          finishedAt: new Date(),
        },
    ...
    const chargeResult = await traceService.withSpan(
      "ai.billing_charge",
      { requestId, traceId },
      async () => creditsService.chargeForAiRequest(requestId)
    );
    ...
    return sendSuccess(
      res,
      {
        reply: {
          ...
          costUsd,
          switchedFromModel:
            originallySelectedModel !== selectedModel ? originallySelectedModel : null,
    ...
    if (isModelNotAllowedError(error)) {
      const details = error instanceof AppError ? (error.details as Record<string, unknown>) : {};
      return sendModelNotAllowed(res, {
        allowedSelections: Array.isArray(details.allowedSelections)
          ? (details.allowedSelections as string[])
          : [aiService.getDefaultSelection()],
        defaultSelection:
          typeof details.defaultSelection === "string"
    ...
    if (isProviderDegradedError(error)) {
      ...
      return sendProviderDegraded(res, {
        ...
        suggestedModels: Array.isArray(details.suggestedModels)
          ? (details.suggestedModels as string[])
          : [],
      });
    }
    if (error instanceof AppError && typeof error.details === "object" && error.details) {
      const details = error.details as { error?: string; reason?: string };
    ...
    await reliabilityService.recordRequestFailure(
      aiService.normalizeModel(String((req.body as Record<string, unknown>).model ?? "openrouter")),
      0,
      error
    );
    ...
    await recordAiUsageEvent({
      userId: req.user?.id,
      provider: aiService.normalizeModel(String((req.body as Record<string, unknown>).model ?? "openrouter")),
      modelName: aiService.normalizeModel(String((req.body as Record<string, unknown>).model ?? "openrouter")),
      status: "ERROR",
      errorCode: "CHAT_ERROR",
    });
    return next(error);
...
export const getAiModels = async (
  ...
  try {
    ...
    const modelConfig = aiService.getModelConfig();
    const billing = await creditsService.assertCanUseAi(req.user.id).catch(() => null);
    const subscriptionActive = await creditsService.hasActiveSubscription(req.user.id);
    const modelSelections = modelConfig.models.flatMap((provider) =>
      (modelConfig.providerVersions[provider] ?? []).map((version) => {
        ...
        return {
          ...
          version,
          label: versionLabel,
          free: aiService.isFreeSelection(id),
    ...
    return sendSuccess(res, {
      ...
      billing: billing ?? {
        subscriptionActive,
        credits: await creditsService.getWalletSummary(req.user.id),
      },
    });
  } catch (error) {
    return next(error);
...
export const postAiChatStream = async (
  ...
  try {
    ...
    if (activeStreamControllers.has(requestId)) {
      throw new AppError(409, "requestId is already active");
    }
    const existing = await prisma.aiRequestLog.findUnique({
      where: { requestId },
      select: {
    ...
    const guarded = await traceService.withSpan(
      "ai.context_build",
      { requestId, traceId, chatId: chatId ?? null, stream: true },
      async () => {
        const rawMessages = parseMessages(body.messages);
        return promptGuardService.enforcePromptGuardrails(rawMessages);
      }
    ...
    streamTimeout = setTimeout(() => {
      controller.abort();
    }, Math.max(5000, env.AI_STREAM_MAX_DURATION_MS));
    ...
    let lastTokenAt: Date | null = null;
    let chunkCount = 0;

    requestStartedAtMs = Date.now();
    const streamResult = await traceService.withSpan(
      "ai.provider_call",
      {
        requestId,
        traceId,
    ...
    await reliabilityService.recordRequestSuccess(selectedModel, Date.now() - requestStartedAtMs);
    const firstTokenAtMs = firstTokenAt ? (firstTokenAt as unknown as Date).getTime() : null;
    const lastTokenAtMs = lastTokenAt ? (lastTokenAt as unknown as Date).getTime() : null;
    const ttftMs =
      firstTokenAtMs !== null ? firstTokenAtMs - requestStartedAtMs : 0;
    const streamDurationMs =
      firstTokenAtMs !== null && lastTokenAtMs !== null
    ...
    await traceService.withSpan("ai.response_write", { requestId, traceId, stream: true }, async () => {
      await prisma.aiRequestLog.update({
        where: { requestId },
        data: {
          traceId,
          ...
          providerUsageRaw: streamResult.providerUsageRaw
            ? (streamResult.providerUsageRaw as Prisma.InputJsonValue)
            : undefined,
          costUsd,
    ...
    if (isProviderDegradedError(error)) {
      ...
      res.write(
        `data: ${JSON.stringify({
          ...
          error: "PROVIDER_DEGRADED",
          message: String(
            details.message ?? "Selected provider is currently degraded. Choose another model."
          ),
          suggestedModels: Array.isArray(details.suggestedModels)
            ? (details.suggestedModels as string[])
            : [],
    ...
    if (cancelled) {
      ...
      await prisma.aiRequestLog
        ...
        .catch(() => undefined);
      if (!res.headersSent) {
        return sendSuccess(res, { requestId, status: "CANCELLED" }, "Stream cancelled");
      }
...
export const getProviderHealth = async (
  ...
  try {
    if (!req.user) throw new AppError(401, "Unauthorized");
    const providers = await reliabilityService.getAllProviderHealth();
...
export const cancelAiChatStream = async (
  ...
  try {
    ...
    if (!requestId) throw new AppError(400, "requestId is required");
    ...
    if (!log) throw new AppError(404, "requestId not found");
    if (log.userId !== req.user.id) throw new AppError(403, "Forbidden");

    const active = activeStreamControllers.get(requestId);
    if (!active) {
      return sendSuccess(
        res,
        { requestId, status: log.status, cancelled: false },
    ...
    return sendSuccess(
      res,
      { requestId, status: "CANCELLED", cancelled: true },
...
export const getAiAdminConfig = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    return sendSuccess(res, { ...aiService.getAdminConfig(), persisted: config });
  } catch (error) {
    return next(error);
  }
};

export const patchAiAdminConfig = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (!reason) {
      throw new AppError(400, "reason is required for config updates");
    }

    const body = (req.body ?? {}) as {
      enabledModels?: Partial<Record<ChatModel, boolean>>;
      multipliers?: Partial<Record<ChatModel, number>>;
      reason?: string;
    };
    const before = await loadAiConfigRow();

    if (body.enabledModels && typeof body.enabledModels === "object") {
      const models = Object.keys(body.enabledModels) as ChatModel[];
      for (const model of models) {
        const value = body.enabledModels[model];
        if (typeof value === "boolean") {
          aiService.setModelEnabled(model, value);
        }
      }
    }

    if (body.multipliers && typeof body.multipliers === "object") {
      const models = Object.keys(body.multipliers) as ChatModel[];
      for (const model of models) {
        const value = body.multipliers[model];
        if (typeof value === "number") {
          aiService.setModelMultiplier(model, value);
        }
      }
    }

    const afterRuntime = aiService.getAdminConfig();
    const after = await (prisma as any).aiConfig.update({
      where: { id: "singleton" },
      data: {
    ...
    await securityService.createConfigAuditLog({
      ...
      userAgent: req.headers["user-agent"],
      requestId:
        String((req.body as Record<string, unknown>)?.requestId ?? "").trim() ||
        `cfg-${Date.now()}`,
      configVersion: after.version as number,
    });
    ...
    return sendSuccess(
      res,
      { ...afterRuntime, persisted: after },
      "AI admin config updated"
    );
  } catch (error) {
    return next(error);
  }
