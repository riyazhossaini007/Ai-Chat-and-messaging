...
const featureGate_service_1 = require("./featureGate.service");
...
const dayKey = (d) => startOfDayUtc(d).toISOString().slice(0, 10);
const buildDaySeries = (from, to, counts) => {
    const out = [];
...
const getCurrentPlanForUser = async (userId) => {
...
const listEntitlements = async (userId) => {
...
const getOverviewStats = async (range) => {
    ...
    const [totalUsers, newUsersRows, newUsers, dau, wau, messages, aiLogs, aiUsers, callRows, paidUsersViaSubs, paidUsersViaUserSubs, complimentaryUsers, reportsOpen, reportsResolved, bansActive,] = await Promise.all([
        prisma_1.prisma.user.count({ where: { status: { not: "DELETED" } } }),
        prisma_1.prisma.user.findMany({
            where: { createdAt: { gte: from, lte: to } },
            select: { createdAt: true },
        }),
        prisma_1.prisma.user.count({ where: { createdAt: { gte: from, lte: to } } }),
        prisma_1.prisma.user.count({ where: { lastActiveAt: { gte: dauStart, lte: to } } }),
        prisma_1.prisma.user.count({ where: { lastActiveAt: { gte: wauStart, lte: to } } }),
        ...
        prisma_1.prisma.aiRequestLog.groupBy({
            by: ["userId"],
        ...
        prisma_1.prisma.userSubscription.groupBy({
            by: ["userId"],
    ...
    const modelBreakdown = { OPENAI: 0, OPENROUTER: 0 };
    aiLogs.forEach((row) => {
        aiRequests += 1;
        ...
        if (provider.includes("OPENROUTER"))
            modelBreakdown.OPENROUTER += 1;
        else
            modelBreakdown.OPENAI += 1;
    ...
    callRows.forEach((row) => {
        ...
        if (["ended", "accepted", "in_progress"].includes(status))
            successfulCalls += 1;
        if (["failed", "declined", "busy", "timeout", "missed"].includes(status))
            failedCalls += 1;
        if (status === "failed" && String(row.failureReason ?? "").toLowerCase().includes("drop"))
            droppedCalls += 1;
    ...
    const paidUserIds = new Set([
        ...
        ...paidUsersViaUserSubs.map((r) => r.userId),
    ]);
    return {
...
const patchUserRole = async (input) => {
    await rbac_service_1.rbacService.ensureRbacBootstrap();
    const existingRoles = await rbac_service_1.rbacService.getUserRoles(input.userId).catch(() => []);
    await Promise.all(existingRoles.filter((r) => r !== input.role).map((r) => rbac_service_1.rbacService.revokeRole({ userId: input.userId, role: r })));
    await rbac_service_1.rbacService.assignRole({ userId: input.userId, role: input.role });
...
const patchUserStatus = async (input) => {
    const updated = await prisma_1.prisma.user.update({
        where: { id: input.userId },
        data: { status: input.status, adminNote: input.reason ? input.reason.slice(0, 1000) : undefined },
...
const patchUserNote = async (input) => {
    const updated = await prisma_1.prisma.user.update({
        where: { id: input.userId },
...
const listAiUsage = async (filters) => {
    const aiUsageRows = (await prismaAny.aiUsageEvent
        ?.findMany?.({
        where: {
        ...
        .catch(() => [])) ?? [];
    const rows = aiUsageRows.length > 0
        ? aiUsageRows.map((r) => ({
            userId: r.userId,
        ...
        : await prisma_1.prisma.aiRequestLog.findMany({
            where: {
                createdAt: { gte: filters.from, lte: filters.to },
                ...(filters.userId ? { userId: filters.userId } : {}),
                ...(filters.provider ? { provider: { contains: filters.provider, mode: "insensitive" } } : {}),
    ...
    const topUsers = topUserIds.length
        ? await prisma_1.prisma.user.findMany({ where: { id: { in: topUserIds } }, select: { id: true, username: true, name: true } })
        : [];
...
const getTopAiUsers = async (input) => {
    const aiUsageRows = (await prismaAny.aiUsageEvent
        ?.findMany?.({
        ...
        .catch(() => [])) ?? [];
    const rows = aiUsageRows.length > 0
        ? aiUsageRows.map((r) => ({ userId: r.userId, totalTokens: (r.tokensIn ?? 0) + (r.tokensOut ?? 0) }))
        : await prisma_1.prisma.aiRequestLog.findMany({
            where: { createdAt: { gte: input.from, lte: input.to } },
    ...
    const sorted = Array.from(map.entries()).sort((a, b) => input.metric === "requests" ? b[1].requests - a[1].requests : b[1].tokens - a[1].tokens);
    const ids = sorted.slice(0, 20).map(([id]) => id);
    const users = ids.length ? await prisma_1.prisma.user.findMany({ where: { id: { in: ids } }, select: { id: true, username: true, name: true } }) : [];
...
const moderationBanUser = async (input) => {
...
const moderationRemoveMessage = async (input) => {
    const message = await prisma_1.prisma.message.findUnique({ where: { id: input.messageId }, select: { id: true } });
    if (!message)
...
const listGroups = async (query) => {
    const rows = await prisma_1.prisma.group.findMany({
        where: {
            ...(query.q
                ? {
                    OR: [
    ...
    return rows
        .filter((g) => (Number.isFinite(minMembers) ? g._count.members >= Number(minMembers) : true))
        .map((g) => ({
        id: g.id,
        title: g.title,
        name: g.name,
...
const getGroupDetail = async (id) => {
    ...
    const recentActivityMessages = await prisma_1.prisma.message.count({
...
const freezeGroup = async (input) => {
    const updated = await prisma_1.prisma.group.update({
        where: { id: input.groupId },
        data: { status: (input.freeze ? "FROZEN" : "ACTIVE") },
...
const softDeleteGroup = async (input) => {
    const updated = await prisma_1.prisma.group.update({
        where: { id: input.groupId },
        data: { status: "DELETED" },
        select: { id: true, status: true },
    ...
    const filtered = rows.filter((row) => {
        const dur = Number(row.durationSec ?? 0) ||
            (row.endedAt && (row.connectedAt ?? row.startedAt)
                ...
                : 0);
        if (durationMin && dur < durationMin)
            return false;
        if (!q)
            return true;
        const hay = [row.id, row.chatId, row.createdBy, row.hostUserId, ...row.participants.map((p) => p.userId)]
            .filter(Boolean)
            .join(" ")
            .toLowerCase();
...
const getCallStats = async (range) => {
    const rows = await prismaAny.callSession.findMany({
    ...
    rows.forEach((r) => {
        ...
        if (["ended", "in_progress", "accepted"].includes(status))
            successCalls += 1;
        if (status === "failed" && r.failureReason)
            failures.set(String(r.failureReason), (failures.get(String(r.failureReason)) ?? 0) + 1);
        if (status === "failed" && String(r.failureReason ?? "").toLowerCase().includes("drop"))
            dropped += 1;
...
exports.adminService = {
    ...
    listAuditLogs,
    getHealth,
    featureGateService: featureGate_service_1.featureGateService,
};
