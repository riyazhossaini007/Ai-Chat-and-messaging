...
const reaction_service_1 = require("../message/reaction.service");
...
const decryptMessageContent = (message, dek) => {
...
const getReadCount = (message) => message.reads.reduce((count, read) => {
    if (read.user.settings?.readReceiptsEnabled === false)
        return count;
    if (!message.senderId)
        return count + 1;
...
const withGroupMessageMeta = (message, dek, overrides) => ({
    ...(() => {
...
const ensureGroupWithChat = async (groupId) => {
    ...
    if (!group) {
        throw new errorHandler_1.AppError(404, "Group not found", { code: GROUP_ERROR_CODES.GROUP_NOT_FOUND });
    }
    return group;
};
const getGroupMembership = async (groupId, userId) => prisma_1.prisma.groupMember.findUnique({
    where: { userId_groupId: { userId, groupId } },
    select: { role: true, groupId: true, userId: true, leftAt: true },
});
const requireGroupRole = async (groupId, userId) => {
    const member = await getGroupMembership(groupId, userId);
    if (!member) {
        await ensureGroupWithChat(groupId);
        throw new errorHandler_1.AppError(403, "Not a group member", {
...
const requireAtLeastAdmin = async (groupId, userId) => {
    const role = await requireGroupRole(groupId, userId);
    if (role !== "ADMIN" && role !== "CREATOR") {
        throw new errorHandler_1.AppError(403, "Insufficient role");
    }
    return role;
};
const requireCreator = async (groupId, userId) => {
    const role = await requireGroupRole(groupId, userId);
    if (role !== "CREATOR") {
        throw new errorHandler_1.AppError(403, "Only creator can perform this action");
    }
    return role;
};
...
const createSystemMessage = async (tx, input) => {
    const encryptedText = (0, messageCrypto_1.encryptText)(input.text, input.dek);
    const message = await tx.message.create({
        data: {
            chatId: input.chatId,
            chatType: client_1.MessageChatType.GROUP,
...
const createGroup = async (creatorId, input) => {
    const title = input.title.trim();
    if (!title)
        throw new errorHandler_1.AppError(400, "title is required");
    const uniqueMemberIds = Array.from(new Set([creatorId, ...(input.memberIds ?? [])].filter(Boolean)));
    const users = await prisma_1.prisma.user.findMany({
        where: { id: { in: uniqueMemberIds } },
        select: { id: true, name: true, username: true },
    });
    if (users.length !== uniqueMemberIds.length) {
        throw new errorHandler_1.AppError(400, "One or more users are invalid");
    }
    ...
    for (const member of users) {
        if (member.id === creatorId)
            continue;
        if (await privacy_guard_1.privacyGuard.isBlocked(creatorId, member.id)) {
            throw new errorHandler_1.AppError(403, `Cannot add ${member.username} due to privacy settings`);
        }
    }
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        await tx.groupMember.createMany({
            data: uniqueMemberIds.map((userId) => ({
                userId,
                groupId: group.id,
                role: userId === creatorId ? client_1.GroupRole.CREATOR : client_1.GroupRole.MEMBER,
            })),
            skipDuplicates: true,
        });
        ...
        if (!created)
            throw new errorHandler_1.AppError(500, "Failed to create group");
        return created;
    });
};
const getMyGroups = async (userId) => {
    ...
    const dekEntries = await Promise.all(Array.from(new Set(memberships.map((membership) => membership.group.chatId))).map(async (chatId) => {
    ...
    const rows = await Promise.all(memberships.map(async (membership) => {
        ...
        const lastMessagePreview = (() => {
            ...
            if (last.kind === "SYSTEM")
                return plainText || "System update";
            const raw = plainText;
            if (!raw)
                return "Media";
            const senderPrefix = last.senderId === userId
                ? "You: "
        ...
        const unseenCount = await prisma_1.prisma.message.count({
            where: {
                ...
                kind: "USER",
                createdAt: {
                    gt: (await prisma_1.prisma.chatParticipant.findUnique({
...
const getGroupDetails = async (groupId, userId) => {
    ...
    return {
        ...
        myRole,
        memberCount: group.members.length,
        invite: myRole === "ADMIN" || myRole === "CREATOR"
            ? group.invites[0] ?? null
            : null,
    };
};
const updateGroup = async (groupId, userId, input) => {
    ...
    return prisma_1.prisma.$transaction(async (tx) => {
        const { dek, encVersion } = await (0, chatEncryption_service_1.getOrCreateChatDek)(group.chatId);
...
const updateRules = async (groupId, userId, rulesText) => {
    await requireAtLeastAdmin(groupId, userId);
    const group = await ensureGroupWithChat(groupId);
    const actor = await prisma_1.prisma.user.findUnique({
        where: { id: userId },
...
const addMembers = async (groupId, userId, userIds) => {
    await requireAtLeastAdmin(groupId, userId);
    const group = await ensureGroupWithChat(groupId);
    const uniqueIds = Array.from(new Set(userIds.filter(Boolean)));
    if (uniqueIds.length === 0)
        throw new errorHandler_1.AppError(400, "userIds is required");
    const actor = await prisma_1.prisma.user.findUnique({
        where: { id: userId },
    ...
    if (!actor)
        throw new errorHandler_1.AppError(404, "User not found");
    const existingMembers = await prisma_1.prisma.groupMember.findMany({
    ...
    if (users.length !== toUpsert.length)
        throw new errorHandler_1.AppError(400, "One or more users are invalid");
    for (const target of users) {
        if (await privacy_guard_1.privacyGuard.isBlocked(userId, target.id)) {
            throw new errorHandler_1.AppError(403, `Cannot add ${target.username} due to privacy settings`);
        }
    }
    return prisma_1.prisma.$transaction(async (tx) => {
        const { dek, encVersion } = await (0, chatEncryption_service_1.getOrCreateChatDek)(group.chatId);
        ...
        if (toReactivate.length > 0) {
            await tx.groupMember.updateMany({
                ...
                data: { leftAt: null, joinedAt: new Date(), role: client_1.GroupRole.MEMBER },
            });
        }
...
const removeMember = async (groupId, actorId, targetUserId) => {
    ...
    if (target.role === "CREATOR" && actorRole !== "CREATOR") {
        throw new errorHandler_1.AppError(403, "Admin cannot remove creator");
    }
    if (target.role === "CREATOR" && actorId === targetUserId) {
        throw new errorHandler_1.AppError(400, "Creator must transfer ownership before leaving");
    }
    const actor = await prisma_1.prisma.user.findUnique({
        where: { id: actorId },
        select: { name: true, username: true },
...
const leaveGroup = async (groupId, userId, input) => {
    const group = await ensureGroupWithChat(groupId);
    const membership = await getGroupMembership(groupId, userId);
    if (!membership) {
        throw new errorHandler_1.AppError(404, "Not a group member", { code: GROUP_ERROR_CODES.NOT_A_MEMBER });
    }
    if (membership.leftAt) {
        throw new errorHandler_1.AppError(400, "Already left group", { code: GROUP_ERROR_CODES.ALREADY_LEFT });
    }
    ...
    return prisma_1.prisma.$transaction(async (tx) => {
        const { dek, encVersion } = await (0, chatEncryption_service_1.getOrCreateChatDek)(group.chatId);
        let newCreatorUserId = null;
        let newCreatorPreviousRole = null;
        if (role === "CREATOR") {
            ...
            if (activeOthers.length === 0) {
                throw new errorHandler_1.AppError(400, "Creator cannot leave as the only active member. Delete the group instead.", {
                    code: GROUP_ERROR_CODES.CREATOR_MUST_ASSIGN_ADMIN,
                });
            }
            const candidateUserId = input?.newAdminUserId?.trim();
            if (!candidateUserId) {
                throw new errorHandler_1.AppError(403, "Creator must assign a new admin before leaving", {
                    code: GROUP_ERROR_CODES.CREATOR_MUST_ASSIGN_ADMIN,
                });
            }
            const transferMember = activeOthers.find((member) => member.userId === candidateUserId);
            if (!transferMember) {
                throw new errorHandler_1.AppError(400, "Selected new admin is not an active member");
            }
            if (candidateUserId === userId)
                throw new errorHandler_1.AppError(400, "Cannot transfer ownership to self");
            ...
            await tx.groupMember.update({
                ...
                data: { role: client_1.GroupRole.CREATOR },
            });
        ...
        return {
            ...
            roleTransfer: newCreatorUserId
                ? {
                    ...
                    newCreatorPreviousRole: newCreatorPreviousRole ?? client_1.GroupRole.MEMBER,
                }
                : null,
        };
    });
};
const transferCreator = async (groupId, creatorId, input) => {
    await requireCreator(groupId, creatorId);
    const group = await ensureGroupWithChat(groupId);
    const nextCreatorUserId = input.newCreatorUserId.trim();
    if (!nextCreatorUserId)
    ...
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (!candidate || candidate.leftAt) {
            throw new errorHandler_1.AppError(400, "Selected member is not active");
        }
        await tx.groupMember.update({
...
const setAdminRole = async (groupId, creatorId, input) => {
    await requireCreator(groupId, creatorId);
    const group = await ensureGroupWithChat(groupId);
    if (input.userId === creatorId)
        throw new errorHandler_1.AppError(400, "Creator role cannot be changed");
    const target = await prisma_1.prisma.groupMember.findUnique({
    ...
    if (target.role === "CREATOR")
        throw new errorHandler_1.AppError(400, "Cannot demote creator");
    ...
    return prisma_1.prisma.$transaction(async (tx) => {
        const { dek, encVersion } = await (0, chatEncryption_service_1.getOrCreateChatDek)(group.chatId);
        const nextRole = input.action === "PROMOTE" ? client_1.GroupRole.ADMIN : client_1.GroupRole.MEMBER;
        await tx.groupMember.update({
            where: { userId_groupId: { userId: input.userId, groupId } },
            data: { role: nextRole },
        });
        const event = input.action === "PROMOTE" ? "ADMIN_PROMOTED" : "ADMIN_DEMOTED";
        const systemMessage = await createSystemMessage(tx, {
            chatId: group.chatId,
            groupId,
            event,
...
const issueInvite = async (groupId, userId, input) => {
    await requireAtLeastAdmin(groupId, userId);
    await ensureGroupWithChat(groupId);
    ...
    if (expiresAt && Number.isNaN(expiresAt.getTime())) {
        throw new errorHandler_1.AppError(400, "Invalid expiresAt");
    }
    const token = (0, crypto_1.randomBytes)(24).toString("hex");
    const invite = await prisma_1.prisma.groupInvite.create({
        data: {
            groupId,
            token,
            createdById: userId,
...
const revokeInvite = async (groupId, userId) => {
    await requireAtLeastAdmin(groupId, userId);
    const latest = await prisma_1.prisma.groupInvite.findFirst({
        where: { groupId, revokedAt: null },
        orderBy: { createdAt: "desc" },
    });
    if (!latest)
        return { ok: true, invite: null };
    const invite = await prisma_1.prisma.groupInvite.update({
        where: { id: latest.id },
        data: { revokedAt: new Date() },
    });
    return { ok: true, invite };
};
const joinByInviteToken = async (token, userId) => {
    const invite = await prisma_1.prisma.groupInvite.findUnique({
        where: { token },
        include: {
            group: {
                select: {
    ...
    if (!invite)
        throw new errorHandler_1.AppError(404, "Invite not found");
    if (invite.revokedAt)
        throw new errorHandler_1.AppError(400, "Invite has been revoked");
    if (invite.expiresAt && invite.expiresAt.getTime() < Date.now()) {
        throw new errorHandler_1.AppError(400, "Invite has expired");
    }
    if (await privacy_guard_1.privacyGuard.isBlocked(userId, invite.createdById)) {
        throw new errorHandler_1.AppError(403, "Cannot join this group due to privacy settings");
    }
    ...
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        const message = await createSystemMessage(tx, {
            ...
            systemActorId: userId,
            ...(await (0, chatEncryption_service_1.getOrCreateChatDek)(invite.group.chatId)),
        });
...
const getGroupMessages = async (groupId, userId, cursor, limit = 50) => {
    ...
    const summaryMap = await (0, reaction_service_1.buildReactionSummaryMap)(sliced.map((item) => item.id), userId);
    const chatDek = await (0, chatEncryption_service_1.getChatDek)(group.chatId);
    const dek = chatDek?.dek ?? null;
    return {
        items: sliced.map((item) => withGroupMessageMeta(item, dek, {
            reactionSummary: summaryMap.get(item.id) ?? [],
        })),
        nextCursor: hasNext ? sliced[sliced.length - 1]?.id ?? null : null,
    };
};
const createGroupMessage = async (groupId, userId, payload) => {
    await requireGroupRole(groupId, userId);
    const group = await ensureGroupWithChat(groupId);
    ...
    if (!["TEXT", "IMAGE", "VIDEO", "FILE"].includes(type)) {
        throw new errorHandler_1.AppError(400, "Invalid mediaType");
    }
    const hasText = body.length > 0;
    const hasMedia = mediaUrl.length > 0;
    if (!hasText && !hasMedia) {
        throw new errorHandler_1.AppError(400, "text or mediaUrl is required");
    }
    let replyToId = payload.replyToId ?? null;
    if (replyToId) {
        const replyTarget = await prisma_1.prisma.message.findFirst({
            where: {
                id: replyToId,
                groupId,
                chatId: group.chatId,
        ...
        if (!replyTarget) {
            throw new errorHandler_1.AppError(400, "Invalid replyToId");
        }
        if (replyTarget.kind !== client_1.MessageKind.USER) {
            throw new errorHandler_1.AppError(400, "Cannot reply to system message");
        }
        replyToId = replyTarget.id;
    }
    return prisma_1.prisma.$transaction(async (tx) => {
        const { dek, encVersion } = await (0, chatEncryption_service_1.getOrCreateChatDek)(group.chatId);
        const encryptedBody = hasText ? (0, messageCrypto_1.encryptText)(body, dek) : null;
        const message = await tx.message.create({
            data: {
                chatId: group.chatId,
                chatType: client_1.MessageChatType.GROUP,
        ...
        if (message.mediaUrl && message.type !== "TEXT") {
            await tx.messageMedia.create({
                data: {
                    ...
                    uploaderId: userId,
                    kind: (0, media_metadata_1.inferMediaKind)(message.type, message.mediaUrl),
                    url: message.mediaUrl,
                    sizeBytes: (0, media_metadata_1.estimateMediaSizeBytes)(message.mediaUrl),
                },
            });
        }
        await tx.chat.update({
        ...
        const summaryMap = await (0, reaction_service_1.buildReactionSummaryMap)([message.id], userId);
...
const getGroupMessagesAround = async (groupId, userId, messageId, windowSize = 20) => {
    await requireGroupRole(groupId, userId);
    const group = await ensureGroupWithChat(groupId);
    const target = await prisma_1.prisma.message.findFirst({
        where: {
            id: messageId,
            groupId,
            chatId: group.chatId,
    ...
    const before = await prisma_1.prisma.message.findMany({
        where: {
            groupId,
            chatId: group.chatId,
    ...
    const after = await prisma_1.prisma.message.findMany({
        where: {
            groupId,
            chatId: group.chatId,
    ...
    const targetMessage = await prisma_1.prisma.message.findFirst({
        where: {
            id: messageId,
            groupId,
            chatId: group.chatId,
    ...
    const summaryMap = await (0, reaction_service_1.buildReactionSummaryMap)(ordered.map((item) => item.id), userId);
    const chatDek = await (0, chatEncryption_service_1.getChatDek)(group.chatId);
    const dek = chatDek?.dek ?? null;
    return {
        targetMessageId: messageId,
        items: ordered.map((item) => withGroupMessageMeta(item, dek, {
            reactionSummary: summaryMap.get(item.id) ?? [],
        })),
    };
};
const markGroupMessagesRead = async (groupId, userId, messageIds) => {
    await requireGroupRole(groupId, userId);
    const group = await ensureGroupWithChat(groupId);
    ...
    const eligibleMessages = await prisma_1.prisma.message.findMany({
        where: {
            id: { in: ids },
            chatId: group.chatId,
            groupId,
    ...
    const readCounts = await prisma_1.prisma.messageRead.groupBy({
        by: ["messageId"],
        where: {
    ...
    const countMap = new Map(readCounts.map((item) => [
        item.messageId,
        Number((item._count?.messageId ?? 0)),
    ]));
...
const getMessageReads = async (messageId, requesterUserId) => {
    ...
    if (!message)
        throw new errorHandler_1.AppError(404, "Message not found");
    if (!message.groupId)
        throw new errorHandler_1.AppError(400, "Seen-by is only available for group messages");
    await requireGroupRole(message.groupId, requesterUserId);
    if (message.kind === client_1.MessageKind.SYSTEM)
        return [];
    const reads = await prisma_1.prisma.messageRead.findMany({
        where: {
            messageId: message.id,
            OR: [
                {
...
const deleteGroup = async (groupId, userId) => {
    ...
    const group = await ensureGroupWithChat(groupId);
    await prisma_1.prisma.chat.delete({ where: { id: group.chatId } });
    return { ok: true };
};
